...
import SdDrawer from './drawer.vue';
import { DrawerApi } from './drawer-api';

const USER_DRAWER_INJECT_KEY = Symbol('VBEN_DRAWER_INJECT');

export function useSdDrawer<
...
>(options: DrawerApiOptions = {}) {
  ...
  const Drawer = defineComponent(
    (props: DrawerProps, { attrs, slots }) => {
      return () =>
        h(SdDrawer, { ...props, ...attrs, drawerApi: extendedApi }, slots);
    },
    {
      inheritAttrs: false,
      name: 'SdDrawer',
...
async function checkProps(api: ExtendedDrawerApi, attrs: Record<string, any>) {
  ...
  for (const attr of Object.keys(attrs)) {
    if (stateKeys.has(attr) && !['class'].includes(attr)) {
      // connectedComponent存在时，不要传入Drawer的props，会造成复杂度提升，如果你需要修改Drawer的props，请使用 useSdDrawer 或者api
      console.warn(
        `[Sd Drawer]: When 'connectedComponent' exists, do not set props or slots '${attr}', which will increase complexity. If you need to modify the props of Drawer, please use useSdDrawer or api.`,
      );
    }
  }
}
