import type { SdButtonProps } from '@sd-core/shadcn-ui';
import type { ClassType } from '@sd-core/typings';
...
export interface ActionButtonOptions extends SdButtonProps {
  [key: string]: any;
  content?: string;
  show?: boolean;
}
...
export type ExtendedFormApi = {
  useStore: <T = NoInfer<SdFormProps>>(
    selector?: (state: NoInfer<SdFormProps>) => T,
  ) => Readonly<Ref<T>>;
} & FormApi;

export interface SdFormAdapterOptions<
  T extends BaseFormComponentType = BaseFormComponentType,
> {
  config?: {
