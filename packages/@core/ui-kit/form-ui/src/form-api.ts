import type { Recordable } from '@sd-core/typings';
import type {
  FormState,
  GenericObject,
  ResetFormOpts,
...
import type { FormActions, FormSchema, SdFormProps } from './types';

import { toRaw } from 'vue';

import { Store } from '@sd-core/shared/store';
import {
  bindMethods,
  isFunction,
  mergeWithArrayOverride,
  StateHandler,
} from '@sd-core/shared/utils';

import { objectPick } from '@vueuse/core';

function getDefaultState(): SdFormProps {
  return {
    actionWrapperClass: '',
    collapsed: false,
    collapsedRows: 1,
...
export class FormApi {
  ...
  private prevState: null | SdFormProps = null;

  // private api: Pick<SdFormProps, 'handleReset' | 'handleSubmit'>;
  public form = {} as FormActions;
  isMounted = false;

  public state: null | SdFormProps = null;

  stateHandler: StateHandler;

  public store: Store<SdFormProps>;

  constructor(options: SdFormProps = {}) {
    const { ...storeState } = options;

    const defaultState = getDefaultState();

    this.store = new Store<SdFormProps>(
      {
        ...defaultState,
        ...storeState,
      },
  ...
  private async getForm() {
    ...
    if (!this.form?.meta) {
      throw new Error('<SdForm /> is not mounted');
    }
    return this.form;
  }
  ...
  setState(
    stateOrFn:
      | ((prev: SdFormProps) => Partial<SdFormProps>)
      | Partial<SdFormProps>,
  ) {
