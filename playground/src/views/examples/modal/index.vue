...
import { Page, useSdModal } from '@sd/common-ui';
...
const [BaseModal, baseModalApi] = useSdModal({
  // 连接抽离的组件
  connectedComponent: BaseDemo,
});

const [AutoHeightModal, autoHeightModalApi] = useSdModal({
  connectedComponent: AutoHeightDemo,
});

const [DragModal, dragModalApi] = useSdModal({
  connectedComponent: DragDemo,
});

const [DynamicModal, dynamicModalApi] = useSdModal({
  connectedComponent: DynamicDemo,
});

const [SharedDataModal, sharedModalApi] = useSdModal({
  connectedComponent: SharedDataDemo,
});

const [FormModal, formModalApi] = useSdModal({
...
<template>
  ...
  >
    <template #extra>
      <DocButton path="/components/common-ui/sd-modal" />
    </template>
    <BaseModal />
    <AutoHeightModal />
    <DragModal />
