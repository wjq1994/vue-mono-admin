...
import { Page, useSdDrawer } from '@sd/common-ui';
...
const [BaseDrawer, baseDrawerApi] = useSdDrawer({
  // 连接抽离的组件
  connectedComponent: BaseDemo,
});

const [AutoHeightDrawer, autoHeightDrawerApi] = useSdDrawer({
  connectedComponent: AutoHeightDemo,
});

const [DynamicDrawer, dynamicDrawerApi] = useSdDrawer({
  connectedComponent: DynamicDemo,
});

const [SharedDataDrawer, sharedDrawerApi] = useSdDrawer({
  connectedComponent: SharedDataDemo,
});

const [FormDrawer, formDrawerApi] = useSdDrawer({
...
<template>
  ...
  >
    <template #extra>
      <DocButton path="/components/common-ui/sd-drawer" />
    </template>
    <BaseDrawer />
    <AutoHeightDrawer />
    <DynamicDrawer />
